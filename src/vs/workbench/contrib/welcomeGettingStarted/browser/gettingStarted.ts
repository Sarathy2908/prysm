...
export class GettingStartedPage extends EditorPane {
			...
			if (category.id === this.currentWalkthrough?.id) {
				...
				badgeelements.forEach(badgeelement => {
					if (step.done) {
						...
						badgeelement.classList.remove(...ThemeIcon.asClassNameArray(Codicon.circleLargeOutline));
						badgeelement.classList.add('complete', ...ThemeIcon.asClassNameArray(Codicon.check));
						badgeelement.setAttribute('aria-label', localize('stepDone', "Checkbox for Step {0}: Completed", step.title));
					}
					else {
						badgeelement.setAttribute('aria-checked', 'false');
						badgeelement.parentElement?.setAttribute('aria-checked', 'false');
						badgeelement.classList.remove('complete', ...ThemeIcon.asClassNameArray(Codicon.check));
						badgeelement.classList.add(...ThemeIcon.asClassNameArray(Codicon.circleLargeOutline));
	...
	private currentMediaType: string | undefined;
	...
	private async buildCategoriesSlide() {
		...
		const header = $('.header', {},
			$('h1.product-name.caption', {}, 'Prysm IDE'), // Force correct title
			$('p.subtitle.description', {}, 'Professional code editing platform') // Force correct subtitle
		);
	...
	private buildCategorySlide(categoryID: string, selectedStep?: string) {
		...
		const buildStepList = () => {
			...
			reset(stepListContainer, ...renderedSteps
				.map(step => {
					const codicon = $('.codicon' + (step.done ? '.complete' + ThemeIcon.asCSSSelector(Codicon.check) : ThemeIcon.asCSSSelector(Codicon.circleLargeOutline)),
						{
